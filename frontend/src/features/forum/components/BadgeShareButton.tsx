/**
 * BadgeShareButton Component
 * Provides social sharing functionality for earned badges
 */
...
const BadgeShareButton: React.FC<BadgeShareButtonProps> = ({ badge, username, className = '' }) => {
  const [showMenu, setShowMenu] = useState(false);
  const { showSuccess } = useToast();

  const shareUrl = `${window.location.origin}/u/${username}/badges/${badge.id}`;
  const shareText = `I just earned the "${badge.name}" badge on Neurmatic! ${badge.description}`;
